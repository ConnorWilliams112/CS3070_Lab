#
# KernelThreadPerformanceTest.py
# CS3070 
#
# created: Spring '16
# updated: 09 Jan 2026
...
from multiprocessing import Queue  #https://docs.python.org/3.13/library/multiprocessing.html
import os
import time
import sys
...
class SL_Program(object):
    ...
    def execute(self):
        start = time.time()
        answer = p7.isPrime(self.candidate)
        finish = time.time()
        self.queue.put( (self.candidate, answer, (finish - start)) )
...
if __name__ == '__main__':
    ...
    program1 = SL_Program('program1', q, 149996387363)
    program2 = SL_Program('program2', q, 149996387377)
    program3 = SL_Program('program3', q, 149996387369)

    p1 = SL_Process.SL_Process(program1)
    p2 = SL_Process.SL_Process(program2)
    p3 = SL_Process.SL_Process(program3)

    print("running, calling for process starts")
    start = time.time()
    p1.start()
    p2.start()
    p3.start()
